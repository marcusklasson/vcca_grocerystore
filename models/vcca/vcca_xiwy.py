...
class VCCA(object):
    ...
    def __init__(self, dim_x, dim_z, dim_labels, dim_i, dim_w, word_to_idx,
                    lambda_x=1.0, lambda_i=1.0, lambda_w=1.0, lambda_y=1.0,
                     n_layers_encoder=1, n_layers_decoder=1, fc_hidden_size=512, n_layers_classifier=1):
        """ VCCA for image features, iconic images, text descriptions, and labels.
        ...
        self.build_graph()
        print("VCCA_xiwy is initialized.")
    ...
    def build_graph(self):
        ...
        self.loss = -tf.reduce_mean(self.kl_div + self.x_rec_loss + self.i_rec_loss + self.caption_rec_loss + self.clf_loss)

        ### Used in training feed_dict
        self.log_var = [self.loss, self.kl_div, self.x_rec_loss, self.i_rec_loss, self.caption_rec_loss,
                        self.clf_loss, self.accuracy]
        ...
        self.val_log_var = [self.loss, self.kl_div, self.x_rec_loss, self.i_rec_loss, self.caption_rec_loss,
                            self.clf_loss, self.val_accuracy]
...
def run_training_epoch(args, data, model, hyperparams, session, train_op=None, shuffle=False, mode='train'):
    ...
    for i in range(n_batches):
        ...
        if mode == 'train':
            ...
            clf_loss += np.sum(train_step_results[6])
            accuracy += np.sum(train_step_results[7])
            ...
            clf_loss += np.sum(val_step_results[5])
            accuracy += np.sum(val_step_results[6])
